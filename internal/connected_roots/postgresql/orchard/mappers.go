...
func toDomain(orchard *Orchards) *connected_roots.Orchards {
...
func toDomainSlice(orchards []*Orchards) []*connected_roots.Orchards {
	orchardsDomain := []*connected_roots.Orchards{}
	for _, orchard := range orchards {
		orchardDomain := toDomain(orchard)
		orchardsDomain = append(orchardsDomain, orchardDomain)
	}
	return orchardsDomain
}

func toDB(orchard *connected_roots.Orchards, id string) *Orchards {
	return &Orchards{
		ID:         id,
		Name:       orchard.Name,
		Location:   orchard.Location,
