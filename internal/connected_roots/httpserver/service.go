...
func (s *Service) setSetup() {
	...
	s.Echo.Debug = s.conf.HTTP.Debug
...
func (s *Service) setMiddlewares() {
	...
	s.Params.RecoverDisabled = !s.conf.Recover
	...
	for {
		...
		case val := <-s.Status:
			...
			case service.Heartbeat:
				...
				hostPort := net.JoinHostPort(s.conf.HTTP.Host, strconv.Itoa(s.conf.HTTP.Port))
				url := fmt.Sprintf("%s://%s/health/alive", "http", hostPort)
				go func() {
					ticker := time.NewTicker(time.Duration(s.conf.HTTP.Health.Frequency) * time.Second)
					for range ticker.C {
